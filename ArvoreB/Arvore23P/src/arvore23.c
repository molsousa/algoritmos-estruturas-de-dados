...
arvore23 split(arvore23 p, int chave, arvore23 subarvore, int* chave_promovida)
{
    ...
    if(chave > p->chave_dir){
        *chave_promovida = p->chave_dir;
        ...
        p->dir = NULL;
        p->num_chaves = 1;
        return cria_no(chave, 0, p->pai, p_aux, subarvore, NULL, 1);
    }
        ...
        *chave_promovida = chave;
        ...
        p->dir = NULL;
        p->num_chaves = 1;
        return cria_no(p->chave_dir, 0, p->pai, subarvore, p_aux, NULL, 1);
    }

    else{
        *chave_promovida = p->chave_esq;
        p_aux = cria_no(p->chave_dir, 0, p->pai, p->meio, p->dir, NULL, 1);
        p->chave_esq = chave;
        p->num_chaves = 1;
        p->dir = NULL;
        p->meio = subarvore;
...
arvore23 cria_no(int chave_esq, int chave_dir, arvore23 pai, arvore23 esq, arvore23 meio, arvore23 dir, int num_chaves)
{
...
{
    if(vazia(r))
        return cria_no(chave, 0, NULL, NULL, NULL, NULL, 1);
        ...
        if(!vazia(aux))
            return cria_no(chave_promovida, 0, NULL, r, aux, NULL, 1);
...
arvore23 redistribuir(arvore23 pai, int pos_filho)
{
    ...
    if(pos_filho == 0){
        ...
        if(!eh_folha(meio)){
            ...
            if(no_underflow->meio != NULL)
                no_underflow->meio->pai = no_underflow;
        ...
        if(esq->num_chaves == 2){
            ...
            if(!eh_folha(esq)){
                ...
                if(no_underflow->esq != NULL)
                    no_underflow->esq->pai = no_underflow;
